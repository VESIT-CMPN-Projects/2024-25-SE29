...
const BirthCertificateForm = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // Submit the document request using our controller
      const { data: documentData, error } = await DocumentController.submitDocumentRequest({
        userId: user.id,
        documentType: 'birth',
      ...
      // Navigate to the track page if successful
      if (documentData) {
        navigate(`/track?type=document&id=${documentData.id}`);
      } else {
